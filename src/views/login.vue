    ...
    <q-page>
      <div class="container">
        ...
        <section class="form-login">
          <div class="form-box">
            <h2>Đăng nhập</h2>
            <q-input class="input" v-model="user.email" type="text" outlined label="Email">
              <template v-slot:prepend>
                <q-icon name="email" />
              </template>
            </q-input>
            <q-input class="input" v-model="user.password" type="password" outlined label="Password" @keyup.enter="handleLogin">
              <template v-slot:prepend>
      ...
      const handleLogin = async () => {
        try {
          ...
          if (userLogin.status == "ok") {
              ...
              router.push({ name: "home" }); // Redirect to the 'Product' page on successful login
           } else {
